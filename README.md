...
> **Scope caveat.** The bullets below describe what CSW observes and
> enforces for **workloads you instrument** (typically: agent-installed
> servers / VMs / supported container environments, plus cloud/asset
> metadata where connectors are authorised). Blind spots still exist —
> unmanaged assets, workloads on unsupported platforms, gaps in agent
> or connector coverage, tuned-down policies, and traffic paths CSW does
> not see — which is exactly why deployments should be validated against
> your inventory, not against marketing language.

- **A complete, current inventory** — for workloads under CSW
  instrumentation — spanning hosts/workloads and (where supported by the
  platform) observed processes and packages plus the network conversations
  those workloads participate in across on‑prem and supported cloud
  footprints (always reconciled against your authoritative CMDB or cloud
  asset inventory — CSW strengthens coverage; it doesn't replace owning
  the asset ledger).
...
- **Continuous vulnerability and behavioural awareness** — for workloads
  under instrumentation — surfaced CVE/exposure posture, anomalies and
  drift versus baselines, and rich export paths into your SIEM and
  incident-response workflows (subject to integrations and tuning).
...
- **Can you enumerate, right now — for workloads you truly instrument —
  every process observed talking to your cardholder data environment —
  and show how that list evolves over something like the last 30 days
  without manual reconstruction?** *(PCI DSS Req 1.2, 11.5)* Your QSA
  asks something in this neighbourhood once per assessment cycle.
...
badly.** At the workload — for instrumented workloads with policy in
enforce mode — segmentation evidence you can export for an assessor often
overlaps heavily with artefacts IR teams scramble to recreate under
stress. Assessors revisit periodically; attackers probe reachable paths
...
- **It operates where the workload lives.** Not at the perimeter, not solely
  from an endpoint-console view — primarily on workloads you explicitly
  instrument (servers / VMs / supported container hosts with the agent —
  supplemented by cloud/asset metadata gathered through authorised
  connectors). Many nuanced compliance-evidence problems around *specific
  process pairs and intra‑host lateral paths* resolve far more cleanly with
  workload-resident conversational telemetry — even though other tooling
  provides adjacent partial coverage.
...
- **Hybrid / multi‑cloud cohesion for supported footprints.** AWS, Azure,
  GCP, on‑prem bare metal / virtualised workloads, supported container
  patterns — converge on consistent policy artefacts where the connectors
  and agents deploy. Coverage still depends on authoritative decisions
  about what you enrol; do not confuse *capable* with *automatically
  ubiquitous*.
...
If after reading any one framework you can answer *"yes, on the workloads
we actually defend with equivalent instrumentation, my current stack gives
me the artefacts on page X with comparable fidelity"* — CSW probably is
not differentiated for that obligation for you today. Ambiguity or heroic
glue code is the pragmatic trigger to revisit scope with your Cisco
account team rather than accumulating more slideware diagrams.
